    ...
    <div class="row">
      <div class="col-12">
        <Card :isTable="true" varient="gray">
          <template #title> Product <i v-show="loadingTable" class="fa fa-spinner fa-spin"></i>
          </template>
...
<th v-show="columns.transport_rate">                  
                  <Filterth
                    ...
                    @input="
                      (value) => {
                        if (typeof value == 'string') filter.transport_rate.value = value;
                      }
                    "
                    :label-click="
                      function () {
            ...
            <tbody>
              <tr v-for="(product, index) in products.data">
                <td v-show="columns.id">{{ product.id }}</td>
                    ...
                    <td v-show="columns.transport_rate">{{ product.transport_rate }}</td>
                <td v-show="columns.action" class="text-right">
                  <Dropdown stay :header="product.name" :id="'productindex' + index">
                    <Button
                      btnDropdown
                      type="button"
                    ...
                    <Button btnDropdown type="button" @click="showModal(product)">
...
<div class="form-group">
          <label for="form_input_unit">Unit</label>
          <input
            v-model="form.unit"
            :disabled="form.processing"
            type="text"
            ...
            aria-describedby="form_input_unit-error"
            aria-invalid="true"
          />
          <span id="form_input_unit-error" class="error invalid-feedback">{{ form.errors.unit }}</span>
        </div>
<div class="form-group">
          <label for="form_input_rate">Rate</label>
          <input
            v-model="form.rate"
            :disabled="form.processing"
            type="text"
            name="rate"
            class="form-control"
            :class="{ 'is-invalid': form.errors.rate }"
            id="form_input_rate"
            placeholder="Product rate"
            aria-describedby="form_input_rate-error"
            aria-invalid="true"
          />
          <span id="form_input_rate-error" class="error invalid-feedback">{{ form.errors.rate }}</span>
        </div>
<div class="form-group">
          <label for="form_input_transport_rate">Transport Rate</label>
          <input
            v-model="form.transport_rate"
            :disabled="form.processing"
            type="text"
            name="transport_rate"
            class="form-control"
            :class="{ 'is-invalid': form.errors.transport_rate }"
            id="form_input_transport_rate"
            placeholder="Product transport rate"
            aria-describedby="form_input_transport_rate-error"
            aria-invalid="true"
          />
          <span id="form_input_transport_rate-error" class="error invalid-feedback">{{ form.errors.transport_rate }}</span>
        </div>
      </form>
    </template>
