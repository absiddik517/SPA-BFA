<template>
    ...
    <div class="row">
      <div class="col-12">
        <Card varient="gray" body-class="p-0">
          <template #title> Worker <i v-show="loadingTable" class="fa fa-spinner fa-spin"></i>
          </template>
          ...
          <div class="row p-2 gy-2">
            ...
            <div class="col-6 d-flex justify-content-end align-items-center">
              ...
              <Dropdown animate stay header="Filter" id="filterworkerDropdown">
                <template #btn>
          ...
          <div class="table-responsive">
            ...
            <tbody>
              <tr v-for="(worker, index) in workers.data">
                <td v-show="columns.id">{{ worker.id }}</td>
                <td v-show="columns.name">{{ worker.name }}</td>
                <td v-show="columns.address">{{ worker.address }}</td>
                <td v-show="columns.phone">{{ worker.phone }}</td>
                <td v-show="columns.selery">{{ worker.selery }}</td>
                <td v-show="columns.action" class="text-right">
                  <Dropdown stay :header="worker.name" :id="'workerindex' + index">
                    <Button
                      btnDropdown
                      type="button"
                      @click="deleteworker(worker.delete_url)"
                    >
                      <i class="fa fa-trash"></i> Delete
                    </Button>
                    <Button btnDropdown type="button" @click="showModal(worker)">
          ...
          <div class="p-2">
            <Pagination @traped="loadingTable = true" :items="workers" />
          </div>
        </Card>
      </div>
    </div>
  </Content>
  <Modal id="worker_worker_create_modal" :title="modalTitle" varient="light">
    <template #body>
      <form @submit.prevent="submit" novalidate="novalidate">
        <Input v-model="form.name" field="name" label="Name" :form="form" />
        <Input v-model="form.address" field="address" label="Address" :form="form" />
        <Input v-model="form.phone" field="phone" label="Phone" :form="form" />
        <Input v-model="form.selery" field="selery" label="Selery" :form="form" />
      </form>
    </template>
  ...
  <DeleteConfirm :deleteUrl="deleteUrl" item="worker"/>
</template>

<script>
import {
  AdminLayout, Modal, Card, Input, DeleteConfirm, Spinner, Dropdown, Pagination, Filterth, Button, Content, useValidateForm,
} from "@/Components";
...
export default {
  ...
  mounted() {
    let form = document.querySelector("#worker_worker_create_modal");
  ...
  methods: {
    ...
    getworkers(filter = {}) {
      ...
      Inertia.get(this.route("worker.index"), filter, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onSuccess: () => {
    ...
    showModal(data = null) {
      ...
      this.modalTitle = data == null ? "Create Worker" : "Update Worker";
    ...
    submit() {
      this.form.clearErrors();
      var url = this.isEditing ? this.editUrl : route("worker.store");
      this.form.post(url, {
        preserveScroll: true,
        onSuccess: () => {
          this.form.reset();
